...
Invoke-WebRequest -UseBasicParsing -Uri http://$RF2CLTIP:$RF2WEBPORT/rest/chat -Method POST -Body "/admin $RF2ADMINPW"


# looping until session is RACE1
$SESSIONSTATE = (Invoke-WebRequest -UseBasicParsing "http://$RF2CLTIP:$RF2WEBPORT/rest/watch/sessionInfo"| ConvertFrom-Json).session
echo "session state"
$SESSIONSTATE
...
# message
Invoke-WebRequest -UseBasicParsing -Uri http://$RF2CLTIP:$RF2WEBPORT/rest/chat -Method POST -Body "DOING REVERSE GRID - DO NOT PRESS DRIVE"
...
Invoke-WebRequest -UseBasicParsing -Uri http://$RF2CLTIP:$RF2WEBPORT/rest/chat -Method POST -Body "Skipping to warmup ..."

while( $SESSIONSTATE -ne "WARMUP" )
 {
  write-host "Waiting for warmup ..."
  write-host "Session state "$SESSIONSTATE
  Invoke-WebRequest -UseBasicParsing -Uri http://$RF2CLTIP:$RF2WEBPORT/rest/chat -Method POST -Body "/callvote nextsession"
  sleep 10
  $SESSIONSTATE = (Invoke-WebRequest -UseBasicParsing "http://$RF2CLTIP:$RF2WEBPORT/rest/watch/sessionInfo"| ConvertFrom-Json).session
 }
...
ForEach ($APICOMMAND in "Reverting the grid ...","/batch reversegrid.ini","Reverting for safety reasons again ...","/batch $RF2ROOT\reversegrid.ini")
 {
    $APICOMMAND
    Invoke-WebRequest -UseBasicParsing -Uri http://$RF2CLTIP:$RF2WEBPORT/rest/chat -Method POST -Body "$APICOMMAND"
    #
...
Invoke-WebRequest -UseBasicParsing -Uri http://$RF2CLTIP:$RF2WEBPORT/rest/chat -Method POST -Body "Going straight to race ..."
Invoke-WebRequest -UseBasicParsing -Uri http://$RF2CLTIP:$RF2WEBPORT/rest/chat -Method POST -Body "/callvote nextsession"
...
Invoke-WebRequest -UseBasicParsing -Uri http://$RF2SRVIP:$RF2WEBPORT/rest/chat -Method POST -Body "Thanks for racing."
Invoke-WebRequest -UseBasicParsing -Uri http://$RF2SRVIP:$RF2WEBPORT/navigation/action/NAV_EXIT -Method POST
